...
try {
    while ($listener.IsListening) {
        ...
        elseif ($method -eq "POST" -and $path -match '^/repos/([^/]+)/([^/]+)/generate$') {
            $owner = $Matches[1]
            $templateRepo = $Matches[2]
            $bodyObj = GetJsonBody $request
            Write-Host "Request body: $(if ($bodyObj) { $bodyObj | ConvertTo-Json -Compress } else { '[null]' })"

            if ($bodyObj -and $bodyObj.owner -and $bodyObj.name -and $bodyObj.description -and $null -ne $bodyObj.private) {
                $statusCode = 201
                $responseJson = @{ message = "Repository created" } | ConvertTo-Json
            } else {
                $statusCode = 422
                $responseJson = @{ message = "Repository creation failed" } | ConvertTo-Json
            }
        }
        else {
            $statusCode = 404
